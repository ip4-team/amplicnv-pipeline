...
def get_phenogramviz_columns(data: pd.DataFrame) -> pd.DataFrame:
...
if __name__ == '__main__':
    ...
    get_phenogramviz_columns(df).to_csv(parsed_args.output, sep='\t', index=False, header=False)

    filtered = df[(df['score'] > parsed_args.min_score) &
                  (df['adjusted p-value (Bonferroni)'] < parsed_args.p_value)]
    get_phenogramviz_columns(filtered).to_csv(parsed_args.filtered_output, sep='\t', index=False, header=False)
